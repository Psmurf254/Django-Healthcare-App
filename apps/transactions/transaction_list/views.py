...
class TransactionListView(TemplateView):
    ...
    def get_total_due(self):
        total_due_amount = Transaction.objects.filter(status='Due').aggregate(total_due=Sum('amount'))
        return float(total_due_amount['total_due']) if total_due_amount['total_due'] is not None else 0.0

    def get_total_paid(self):
        total_paid_amount = Transaction.objects.filter(status='Paid').aggregate(total_paid=Sum('amount'))
    ...
    def get_total_canceled(self):
        total_canceled_amount = Transaction.objects.filter(status='Canceled').aggregate(total_canceled=Sum('amount'))
        return float(total_canceled_amount['total_canceled']) if total_canceled_amount[
                                                                     'total_canceled'] is not None else 0.0
