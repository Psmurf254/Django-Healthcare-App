...
$(function () {
  ...
  var dt_notification_table = $('.datatables-notifications'),
    select2 = $('.select2'),
    notificationView = '/app/notification/view/account/',
    statusObj = {
  ...
  if (dt_notification_table.length) {
    var dt_notification = dt_notification_table.DataTable({
      ...
      columnDefs: [
        ...
        {
          // notification full name and email
          targets: 1,
          responsivePriority: 4,
          render: function (data, type, full, meta) {
            var $name = full['notification'],
              $image = full['avatar'];
            if ($image) {
              // For Avatar image
              var $output =
            ...
            var $row_output =
              '<div class="d-flex justify-content-start align-items-center notification-name">' +
        ...
        //     var roleBadgeObj = {
        //       Subscriber:
        //         '<span class="badge badge-center rounded-pill bg-label-warning w-px-30 h-px-30 me-2"><i class="ti ti-notification ti-sm"></i></span>',
        //       Author:
                ...
                format: {
                  body: function (inner, coldex, rowdex) {
                    ...
                    $.each(el, function (index, item) {
                      if (item.classList !== undefined && item.classList.contains('notification-name')) {
      ...
      initComplete: function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="UserRole" class="form-select text-capitalize"><option value=""> Gender Filter </option></select>'
            )
              .appendTo('.notification_role')
              .on('change', function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="UserPlan" class="form-select text-capitalize"><option value=""> Select Contact </option></select>'
            )
              .appendTo('.notification_plan')
              .on('change', function () {
                var val = $.fn.dataTable.util.escapeRegex($(this).val());
                column.search(val ? '^' + val + '$' : '', true, false).draw();
              });
  ...
  // Delete Record
  $('.datatables-notifications tbody').on('click', '.delete-record', function () {
    dt_notification.row($(this).parents('tr')).remove().draw();
  });
